/**
 * A pasta `docs/` da raiz do repositorio guarda duas coisas: as apolices e os
 * manuais que a frota le, e `docs/planos/`, que e planejamento interno em
 * markdown. A rota `/docs/*` serve a primeira e nao pode entregar a segunda, nem
 * nada que esteja fora da pasta.
 *
 * Este arquivo prova as duas barreiras que separam as duas, uma de cada vez: a
 * contencao de caminho (`dentroDe`) isolada, e o comportamento da rota montada no
 * app de verdade. Ele nasceu separado de `documentos.test.ts` porque aquele
 * arquivo passou a cobrir o cadastro de documentos no banco, que e outro assunto:
 * a rota estatica le disco e nao tem cadastro nenhum.
 */
